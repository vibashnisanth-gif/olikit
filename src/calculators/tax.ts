...
// ponytail: brackets hardcoded to 2025-2026 tax year. taxYear param accepted for API
// consistency; add year-specific bracket sets when/if older years are needed.
...
export class TaxCalculator implements Calculator<TaxInput, TaxOutput> {
  ...
  calculate(input: TaxInput): TaxOutput {
    ...
    const brackets = getBrackets(
      locale,
      filingStatus,
      (input as TaxInput & {taxYear?: number}).taxYear
    );
