...
export const SalaryRankingChart = memo(function SalaryRankingChart() {
    ...
    <section className="rounded-xl border border-zinc-200 bg-white px-6 py-8 shadow-sm sm:px-10 sm:py-10">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm leading-6 text-zinc-500">
          Enter your annual salary in USD and see how much you would keep after income tax in each
          country.
        </p>
      </div>

      <div className="mb-6 flex items-center gap-2">
        <label htmlFor="salary-input" className="text-sm text-zinc-500">
          $
        </label>
        <input
          id="salary-input"
      ...
      <div className="divide-y divide-zinc-100">
        {results.map((r, i) => {
          ...
          return (
            <div
              key={r.slug}
              className="group flex items-center gap-3 py-3 transition-colors hover:bg-zinc-50 -mx-3 px-3 rounded-lg"
            >
              <span
                className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ring-1 ring-inset ${i < 3 ? RANK_STYLES[i] : "bg-zinc-50 text-zinc-500 ring-zinc-200"}`}
              >
                {i + 1}
              </span>
              ...
              <div
                className="h-1.5 w-20 shrink-0 overflow-hidden rounded-full bg-zinc-100 sm:w-24"
                aria-hidden="true"
              >
                <div
      ...
      <div className="mt-6 rounded-lg bg-blue-50 px-4 py-3 text-xs leading-5 text-zinc-600 ring-1 ring-blue-100">
        <p>
          <strong className="font-semibold text-zinc-700">How it works:</strong> Each country
          applies its own income tax brackets to your salary. Singapore has no personal income tax
          on the first S$20,000. The US applies a standard deduction of $14,600 before tax. The UK
          allows £12,570 tax-free. This tool computes your estimated take-home pay using 2025-2026
          statutory tax brackets for single filers.
        </p>
        <p className="mt-2">
          Compare countries:{" "}
          <a href="/compare" className="font-medium text-blue-600 hover:text-blue-700">
            Salary Comparison
          </a>{" "}
          ·{" "}
          <a href="/rankings" className="font-medium text-blue-600 hover:text-blue-700">
            Salary Rankings
          </a>{" "}
          ·{" "}
          <a
      ...
      <p className="mt-4 text-[11px] text-zinc-400">
        Estimates only — consult a tax professional for binding advice. Tax brackets sourced from
        IRS, HMRC, ATO, CRA, IRD, IRAS.
      </p>
      <p className="mt-1 text-[10px] text-zinc-300">
        Exchange rates last updated: {EXCHANGE_RATES_LAST_UPDATED}
      </p>
    </section>
  );
});
