...
export function Header({ currentSlug }: Props) {
  ...
  return (
    <header className="sticky top-8 z-30 border-b border-border-light bg-white/95 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        ...
        <div className="hidden lg:flex items-center gap-1 text-sm">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-md px-3 py-2 text-text-secondary transition-colors hover:bg-surface-muted hover:text-text-primary"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-md p-3 text-text-secondary transition-colors duration-150 hover:bg-surface-muted hover:text-text-primary"
            aria-label="Search"
          >
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-md p-3 text-text-secondary hover:bg-surface-muted hover:text-text-primary lg:hidden"
        ...
        <div className="border-t border-border-light bg-white px-4 py-3">
          <form onSubmit={handleSearch} className="mx-auto flex max-w-2xl gap-2">
            <input
              ...
              className="flex-1 rounded-md border border-border-medium px-3 py-2 text-sm outline-none focus:border-gray-400 focus:ring-1 focus:ring-gray-300"
              autoFocus
            />
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 right-0 top-full z-50 max-h-[80vh] overflow-y-auto border-b border-border-light bg-white shadow-lg lg:hidden">
            <div className="flex flex-col gap-1 px-4 py-3">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="rounded-md px-3 py-2 text-sm font-medium text-text-secondary transition-colors duration-150 hover:bg-surface-muted"
