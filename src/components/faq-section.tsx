      ...
      <div className="divide-y divide-zinc-200 rounded-lg border border-zinc-200 bg-white">
        {faqs.map((faq, i) => (
          <details key={i} className="group">
            <summary className="flex cursor-pointer items-center justify-between px-5 py-4 text-sm font-medium text-zinc-950 hover:bg-zinc-50">
              {faq.question}
            ...
            <p className="px-5 pb-4 text-sm leading-7 text-zinc-600">{faq.answer}</p>
          </details>
        ))}
      </div>
    </section>
