...
export const CountrySwitcher = memo(function CountrySwitcher({
  currentSlug,
  currentName,
  countries,
}: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className="absolute right-0 top-full z-50 mt-1 max-h-72 w-52 overflow-y-auto rounded-lg border border-zinc-200 bg-white py-1 shadow-lg"
          role="listbox"
        >
          {allOptions.map((opt) => {
            ...
            return (
              ...
              >
                {opt.slug ? (
                  <FlagImage code={opt.slug} size="sm" />
                ) : (
                  <span
                    className="inline-flex items-center justify-center w-5 h-[15px] text-xs"
                    role="img"
                    aria-label="Global"
                  >
                    🌍
