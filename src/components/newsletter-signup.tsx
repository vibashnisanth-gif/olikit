...
export const NewsletterSignup = memo(function NewsletterSignup({
  ...
  const inputBase =
    "min-w-0 rounded-md border border-border-medium bg-white px-4 py-2 text-sm text-text-primary placeholder-text-muted focus:border-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-300";
  ...
  if (variant === "banner") {
    return (
      <div className={`${cardBase} px-6 py-8 my-6`}>
        ...
        {status === "error" && (
          <p className="mt-2 text-center text-xs text-red-600" role="alert" aria-live="assertive">
            {message}
          </p>
        )}
      </div>
