...
import type {
  ...
  LoanAmortizationInput, LoanAmortizationOutput,
  MortgageInput, MortgageOutput,
  RetirementSavingsInput, RetirementSavingsOutput,
...
const calculatorConfigs: Record<string, SupportedCalculatorConfig> = {
  'salary-calculator': defineCalculatorConfig<SalaryInput, SalaryOutput>({
    ...
    fields: [
      ...
      { name: 'period', label: 'Pay Period', type: 'select', defaultValue: 'annual', options: [
      ...
      { name: 'hoursPerWeek', label: 'Hours per Week', type: 'number', defaultValue: 40, min: 1, max: 168, step: 1 },
      { name: 'daysPerWeek', label: 'Days per Week', type: 'number', defaultValue: 5, min: 1, max: 7, step: 1 },
    ],
  ...
  'tax-calculator': defineCalculatorConfig<TaxInput, TaxOutput>({
    ...
    fields: [
      ...
      { name: 'taxYear', label: 'Tax Year', type: 'number', defaultValue: 2026, min: 2025, max: 2027, step: 1 },
    ],
  ...
  'mortgage-calculator': defineCalculatorConfig<MortgageInput, MortgageOutput>({
    ...
    fields: [
      ...
      { name: 'annualRate', label: 'Annual Interest Rate', type: 'percentage', defaultValue: 7, min: 0, max: 30, step: 0.1 },
      { name: 'termYears', label: 'Loan Term (Years)', type: 'number', defaultValue: 30, min: 1, max: 50, step: 1 },
      { name: 'annualPropertyTax', label: 'Annual Property Tax', type: 'currency', defaultValue: 6000, min: 0 },
      { name: 'annualInsurance', label: 'Annual Insurance', type: 'currency', defaultValue: 1200, min: 0 },
      ...
      return [
        {
          ...
          items: [
            { label: 'Principal & Interest', value: fmtCurrency(output.monthlyPrincipalAndInterest, locale) },
            ...
            { label: 'Total Monthly', value: fmtCurrency(output.totalMonthlyPayment, locale), highlight: true },
          ],
        },
        {
          title: 'Loan Summary',
  ...
  'investment-calculator': defineCalculatorConfig<CompoundInterestInput, CompoundInterestOutput>({
    ...
    fields: [
      { name: 'principal', label: 'Initial Investment', type: 'currency', defaultValue: 10000, min: 0 },
      { name: 'monthlyContribution', label: 'Monthly Contribution', type: 'currency', defaultValue: 500, min: 0 },
      { name: 'annualRate', label: 'Annual Return Rate', type: 'percentage', defaultValue: 8, min: 0, max: 100, step: 0.1 },
      { name: 'years', label: 'Investment Period (Years)', type: 'number', defaultValue: 10, min: 1, max: 100, step: 1 },
      { name: 'compoundsPerYear', label: 'Compounds per Year', type: 'select', defaultValue: 12, options: [
    ...
    formatResult(output: CompoundInterestOutput, locale: CalculatorLocale): ResultGroup[] {
      return [{
        ...
        items: [
          { label: 'Future Value', value: fmtCurrency(output.futureValue, locale), highlight: true },
  ...
  'retirement-calculator': defineCalculatorConfig<RetirementSavingsInput, RetirementSavingsOutput>({
    ...
    fields: [
      ...
      { name: 'retirementAge', label: 'Retirement Age', type: 'number', defaultValue: 65, min: 30, max: 90, step: 1 },
      { name: 'lifeExpectancy', label: 'Life Expectancy', type: 'number', defaultValue: 90, min: 50, max: 120, step: 1 },
      { name: 'currentSavings', label: 'Current Savings', type: 'currency', defaultValue: 50000, min: 0 },
      { name: 'monthlyContribution', label: 'Monthly Contribution', type: 'currency', defaultValue: 1000, min: 0 },
      { name: 'expectedReturnRate', label: 'Expected Return Rate', type: 'percentage', defaultValue: 7, min: 0, max: 30, step: 0.1 },
      { name: 'currentAnnualIncome', label: 'Current Annual Income', type: 'currency', defaultValue: 80000, min: 0 },
      { name: 'desiredReplacementRate', label: 'Desired Income Replacement Rate', type: 'percentage', defaultValue: 80, min: 0, max: 100, step: 1 },
      { name: 'inflationRate', label: 'Inflation Rate', type: 'percentage', defaultValue: 3, min: 0, max: 20, step: 0.1 },
    ],
    formatResult(output: RetirementSavingsOutput, locale: CalculatorLocale): ResultGroup[] {
      return [{
        title: 'Retirement Outlook',
        items: [
          { label: 'Savings at Retirement', value: fmtCurrency(output.savingsAtRetirement, locale), highlight: true },
          { label: 'Monthly Retirement Income', value: fmtCurrency(output.monthlyRetirementIncome, locale) },
          ...
          { label: 'Status', value: output.isOnTrack ? 'On Track' : 'Behind Target', highlight: output.isOnTrack },
  ...
  'business-loan-calculator': defineCalculatorConfig<LoanAmortizationInput, LoanAmortizationOutput>({
    ...
    fields: [
      ...
      { name: 'annualRate', label: 'Annual Interest Rate', type: 'percentage', defaultValue: 8, min: 0, max: 30, step: 0.1 },
      { name: 'termYears', label: 'Loan Term (Years)', type: 'number', defaultValue: 5, min: 1, max: 50, step: 1 },
      { name: 'paymentsPerYear', label: 'Payments per Year', type: 'select', defaultValue: 12, options: [
        { label: 'Monthly (12)', value: 12 },
        { label: 'Quarterly (4)', value: 4 },
        { label: 'Semi-annually (2)', value: 2 },
        { label: 'Annually (1)', value: 1 },
      ...
      return [
        {
          ...
          items: [
            { label: 'Payment per Period', value: fmtCurrency(output.monthlyPayment, locale), highlight: true },
            { label: 'Total Payment', value: fmtCurrency(output.totalPayment, locale) },
            { label: 'Total Interest', value: fmtCurrency(output.totalInterest, locale) },
  ...
  'break-even-calculator': defineCalculatorConfig<BreakEvenInput, BreakEvenOutput>({
    ...
    fields: [
      ...
      { name: 'variableCostPerUnit', label: 'Variable Cost per Unit', type: 'currency', defaultValue: 10, min: 0 },
      { name: 'sellingPricePerUnit', label: 'Selling Price per Unit', type: 'currency', defaultValue: 30, min: 0 },
    ],
    formatResult(output: BreakEvenOutput, locale: CalculatorLocale): ResultGroup[] {
      return [{
        title: 'Break-Even Analysis',
        items: [
          { label: 'Break-Even Units', value: output.breakEvenUnits === Infinity ? 'Never' : fmtBreakEvenUnits(output.breakEvenUnits, locale), highlight: true },
          { label: 'Break-Even Revenue', value: fmtCurrency(output.breakEvenRevenue, locale) },
          { label: 'Contribution Margin per Unit', value: fmtCurrency(output.contributionMarginPerUnit, locale) },
