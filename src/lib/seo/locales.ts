...
export const locales: Locale[] = [
  {
    ...
    googleAdsId: process.env.GOOGLE_ADSENSE_ID || "ca-pub-xxxxxxxx",
...
export function getSubRegion(locale: Locale, slug: string): SubRegion | undefined {
  return locale.states?.find((s) => s.slug === slug);
}

export function getPrimaryLocales(): Locale[] {
  return locales.filter((l) => ["us", "uk", "au", "ca", "nz"].includes(l.slug));
}

export function getExpansionLocales(): Locale[] {
  return locales.filter((l) => ["in", "sg"].includes(l.slug));
}

export function getLocalesWithSubRegion(subregionSlug: string): Locale[] {
  return locales.filter((l) => l.states?.some((s) => s.slug === subregionSlug));
}
