...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Top US states ranked by lowest cost of living. Compare housing, utilities, food, transportation, and healthcare costs across states.",
    ...
    <div className="space-y-10">
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            Best States for Salary
          </Link>
          <Link
            href={`/${locale.slug}/best-states-for-retirement`}
            className="rounded-md bg-white px-4 py-2 text-sm font-medium text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-50"
          >
            Best States for Retirement
          </Link>
          <Link
            href={`/${locale.slug}/best-states-for-home-affordability`}
            className="rounded-md bg-white px-4 py-2 text-sm font-medium text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-50"
          >
            Best States for Home Affordability
          </Link>
        </div>
      </section>
