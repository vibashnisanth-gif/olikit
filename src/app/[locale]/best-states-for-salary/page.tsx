...
import {
  getBestStatesForSalary,
  getBestStatesForCostOfLiving,
  getBestStatesForRetirement,
  getBestStatesForHomeAffordability,
} from "@/lib/content/state-rankings";
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Detailed US state rankings by average salary, cost of living, retirement friendliness, and home affordability. Compare all 50 states with official data.",
    alternates: {canonical: `${SITE_URL}/${locale.slug}/best-states-for-salary`},
    openGraph: {
      title: "US State Rankings",
      description: "Compare states by salary, cost of living, retirement, and home affordability.",
      siteName: "Olikit",
      type: "website",
    },
  };
          ...
          {data.rankings.map((r) => (
            <tr key={r.stateSlug} className="border-b border-zinc-100 last:border-0">
              ...
              <td className="px-4 py-3">
                <Link
                  href={`/${localeSlug}/salary-vs-cost-of-living/${r.stateSlug}`}
    ...
    <div className="space-y-16">
      <div className="rounded-lg border border-zinc-200 bg-white px-5 py-7 shadow-sm sm:px-8">
        <h1 className="max-w-4xl text-3xl font-bold tracking-tight text-zinc-950 sm:text-4xl">
          US State Rankings (2025-2026)
        </h1>
        <p className="mt-4 max-w-3xl text-lg leading-8 text-zinc-600">
          Detailed rankings of US states by average salary, cost of living, retirement friendliness,
          and home affordability. All data sourced from official government publications and updated
          for 2025-2026.
        </p>
      </div>
      ...
      <nav className="flex flex-wrap gap-2">
        {[
          ...
          {
            label: "Best States for Home Affordability",
            href: "#best-states-for-home-affordability",
          },
        ].map((link) => (
          <a
            key={link.href}
      ...
      <RankingSection
        title={salaryRanking.h1}
        id="best-states-for-salary"
        data={salaryRanking}
        localeSlug={locale.slug}
      />
      <RankingSection
        title={colRanking.h1}
        id="best-states-for-cost-of-living"
        data={colRanking}
        localeSlug={locale.slug}
      />
      <RankingSection
        title={retirementRanking.h1}
        id="best-states-for-retirement"
        data={retirementRanking}
        localeSlug={locale.slug}
      />
      <RankingSection
        title={homeRanking.h1}
        id="best-states-for-home-affordability"
        data={homeRanking}
        localeSlug={locale.slug}
      />
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            href={`/${locale.slug}/research`}
            className="rounded-md bg-white px-4 py-2 text-sm font-medium text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-50"
          >
            Research Reports
          </Link>
          <Link
            href={`/${locale.slug}/financial-data`}
            className="rounded-md bg-white px-4 py-2 text-sm font-medium text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-50"
          >
            Financial Data
          </Link>
