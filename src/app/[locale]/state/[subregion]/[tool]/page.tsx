...
import {
  ...
  getAllInternalLinks,
  getSubRegionLinks,
} from "@/lib/linking/internal-links";
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return buildMetadata(
    locale,
    tool,
    `/${locale.slug}/state/${subRegion.slug}/${tool.slug}`,
    subRegion,
    validLocaleSlugs
  );
}
    ...
    <div>
      ...
      {content.steps && content.steps.length > 0 && (
        <section className="mb-10 rounded-lg border border-zinc-200 bg-white px-6 py-7 shadow-sm">
          <h2 className="mb-1 text-xl font-semibold text-zinc-950">How to Use the {toolName}</h2>
      ...
      <div className="grid gap-8 md:grid-cols-3">
          ...
          {dataPoints && (
            <section>
              ...
              <p className="mt-2 text-xs text-zinc-500">
                Sources: Bureau of Labor Statistics, US Census Bureau, state revenue departments
                (2024). Figures are estimates and may vary by source and year.
              </p>
            </section>
          )}
        </div>
        ...
        <aside className="space-y-6">
          ...
          {stateLinks.length > 0 && (
            <div className="p-4 rounded-lg border border-zinc-200">
              <h3 className="font-semibold mb-3">{toolName} by State</h3>
              <div className="space-y-2">
          ...
          <div className="p-4 rounded-lg border border-zinc-200">
            ...
            <div className="space-y-2">
              {guides
                .filter(
                  (g) => g.relatedToolSlugs.includes(tool.slug) || g.category === tool.category
                )
                .map((g) => (
                  <a
                    key={g.slug}
