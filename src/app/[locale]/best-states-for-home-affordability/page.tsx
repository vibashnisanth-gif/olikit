...
import {getBestStatesForHomeAffordability} from "@/lib/content/state-rankings";
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  if (!locale) return {};
  return {
    title: "Best States for Home Affordability (2025-2026)",
    description:
      "Top US states ranked by home affordability. Compare home price-to-income ratios, mortgage affordability, and property taxes.",
    alternates: {canonical: `${SITE_URL}/${locale.slug}/best-states-for-home-affordability`},
  };
}

export default async function BestStatesForHomeAffordabilityPage({params}: Props) {
  const {locale: localeSlug} = await params;
  const locale = getLocale(localeSlug);
  if (!locale) notFound();
  const data = getBestStatesForHomeAffordability();
    ...
    <div className="space-y-10">
            ...
            {data.rankings.map((r) => (
              <tr key={r.stateSlug} className="border-b border-zinc-100 last:border-0">
                ...
                <td className="px-4 py-3">
                  <Link
                    href={`/${locale.slug}/cost-of-living/${r.stateSlug}`}
      ...
      <section className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            href={`/${locale.slug}/best-states-for-salary`}
