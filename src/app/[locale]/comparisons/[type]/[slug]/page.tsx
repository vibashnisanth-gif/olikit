...
import {generateComparisonPage, comparisonPairs} from "@/lib/content/comparison-engine";
import {
  generateProfessionComparison,
  professionComparisonPairs,
  parseProfessionComparisonSlug,
} from "@/lib/content/profession-comparisons";
...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  if (type === "profession-salary") {
    ...
    return {
      ...
      openGraph: {
        title: pc.h1,
        description: pc.metaDescription,
        url: `${SITE_URL}/${locale.slug}/comparisons/${type}/${slug}`,
        siteName: "Olikit",
        locale: locale.code,
        type: "article",
      },
  ...
  if (!pair) return {};
  const content = generateComparisonPage(
    pair.type,
    pair.pairA,
    pair.pairB,
    pair.regionA,
    ...
    return (
      <div className="space-y-10">
        ...
        <nav className="text-sm text-zinc-500">
          <a href={`/${locale.slug}`} className="hover:text-zinc-800">
            Home
          </a>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-blue-200 bg-blue-50 px-5 py-4">
            ...
            <p className="mt-1 text-lg font-bold text-zinc-900">
              {formatSalaryInline(pc.salaryA, pc.symbolA)}
            </p>
          ...
          <div className="rounded-lg border border-indigo-200 bg-indigo-50 px-5 py-4">
            ...
            <p className="mt-1 text-lg font-bold text-zinc-900">
              {formatSalaryInline(pc.salaryB, pc.symbolB)}
            </p>
            <p className="text-sm text-zinc-600">Average annual salary</p>
          </div>
        </div>
        ...
        <KeyTakeaways
          items={pc.keyTakeaways}
          title={`${pc.professionName} Salary Comparison Highlights`}
        />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50">
                <th scope="col" className="px-4 py-3 text-left font-semibold text-zinc-700">
                  Metric
                </th>
                <th scope="col" className="px-4 py-3 text-left font-semibold text-blue-700">
                  {pc.localeA.name}
                </th>
                <th scope="col" className="px-4 py-3 text-left font-semibold text-indigo-700">
                  {pc.localeB.name}
                </th>
              </tr>
            </thead>
            ...
            <div className="flex flex-wrap gap-3">
              {pc.relatedComparisons.map((link, i) => (
                <a
                  key={i}
                  href={link.href}
                  className="rounded-md bg-zinc-100 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-blue-100 hover:text-blue-700 transition-colors"
                >
                  {link.label}
                </a>
              ))}
        ...
        <NewsletterSignup
          locale={localeSlug}
          source={`profession-compare-${slug}`}
          variant="banner"
        />
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      {"@type": "ListItem", position: 1, name: locale.name, item: `${SITE_URL}/${locale.slug}`},
      {
        "@type": "ListItem",
        position: 2,
        name: `${typeLabel} Comparisons`,
        item: `${SITE_URL}/${locale.slug}/comparisons/${type}`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: content.h1,
        item: `${SITE_URL}/${locale.slug}/comparisons/${type}/${slug}`,
      },
    ],
  };
  ...
  return (
    <div className="space-y-10">
      ...
      <nav className="text-sm text-zinc-500">
        ...
        <span className="mx-2">/</span>
        <a href={`/${locale.slug}/comparisons`} className="hover:text-zinc-800">
          Comparisons
        </a>
        <span className="mx-2">/</span>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-blue-200 bg-blue-50 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-600">
            {content.localeA.name}
          </p>
          <p className="mt-1 text-lg font-bold text-zinc-900">
            {content.comparisonTable[0]?.valueA ?? content.localeA.name}
          </p>
          {content.regionA && (
            <p className="text-sm text-zinc-600">Region: {content.regionA.name}</p>
          )}
        </div>
        <div className="rounded-lg border border-indigo-200 bg-indigo-50 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-indigo-600">
            {content.localeB.name}
          </p>
          <p className="mt-1 text-lg font-bold text-zinc-900">
            {content.comparisonTable[0]?.valueB ?? content.localeB.name}
          </p>
          {content.regionB && (
            <p className="text-sm text-zinc-600">Region: {content.regionB.name}</p>
          )}
        </div>
      </div>
      ...
      <section className="overflow-x-auto rounded-lg border border-zinc-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          <caption className="sr-only">
            Comparison of {content.localeA.name} vs {content.localeB.name}
          </caption>
          <thead>
            <tr className="border-b border-zinc-200 bg-zinc-50">
              <th scope="col" className="px-4 py-3 text-left font-semibold text-zinc-700">
      ...
      {type === "purchasing-power" && (
        <section className="rounded-lg border border-blue-200 bg-blue-50 px-6 py-5">
          <h2 className="text-xl font-semibold text-blue-600 mb-3">Purchasing Power Analysis</h2>
          <p className="text-blue-600 leading-relaxed">
            A salary {content.regionA ? `in ${content.regionA.name}` : `in ${content.localeA.name}`}{" "}
            has different real-world value when adjusted for local costs. The effective purchasing
            power tells you what your salary is actually worth.
          </p>
          {content.salaryDiff && (
            <p className="mt-2 text-blue-600">
              <strong>Salary:</strong> {content.salaryDiff}
            </p>
          )}
          {content.colDiff && (
            <p className="mt-2 text-blue-600">
              <strong>Cost of Living:</strong> {content.colDiff}
            </p>
          )}
          {content.purchasingPowerDiff && (
            <p className="mt-2 text-blue-600">
              <strong>Purchasing Power:</strong> {content.purchasingPowerDiff}
            </p>
          )}
        </section>
      )}
