...
export default async function CityProfessionPage({params}: Props) {
  const {locale, city, profession: professionSlug} = await params;
  const localeData = getLocale(locale);
  ...
  const breadcrumbSchema = buildBreadcrumbJsonLd([
    ...
    {label: `${profession.name} Salary`, url: `${SITE_URL}/${locale}/salary/${professionSlug}`},
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        <nav className="mb-6 text-sm text-gray-500">
          ...
          <span className="mx-2">/</span>
          <Link href={`/${locale}/salary/${professionSlug}`} className="hover:text-gray-700">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Link
              href={`/${locale}/salary/${professionSlug}`}
