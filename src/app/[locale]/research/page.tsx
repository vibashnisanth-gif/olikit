...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Data-driven research reports on US salaries, cost of living, taxes, housing affordability, and salary growth trends. All findings based on official government data.",
    alternates: {canonical: `${SITE_URL}/${locale.slug}/research`},
    openGraph: {
      title: "Research & Insights Center - Olikit",
      description: "Data-driven financial research reports.",
    ...
    <div className="space-y-10">
      <div className="rounded-lg border border-zinc-200 bg-white px-5 py-7 shadow-sm sm:px-8">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-zinc-600">
          Data-driven research reports based exclusively on official government data. Every report
          includes methodology, sources, and key findings. No fabricated data, no unsubstantiated
          claims.
        </p>
      </div>
      ...
      <div className="grid gap-6">
        {researchReports.map((report) => (
          <Link
            key={report.slug}
            href={`/${locale.slug}/research/${report.slug}`}
