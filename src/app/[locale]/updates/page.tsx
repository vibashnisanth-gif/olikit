...
export async function generateMetadata({params}: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Stay up to date with the latest tax updates, salary data changes, mortgage rate updates, and new content on Olikit.",
    ...
    <div className="space-y-10">
      <div className="rounded-lg border border-zinc-200 bg-white px-5 py-7 shadow-sm sm:px-8">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-zinc-600">
          Stay informed about the latest tax updates, salary data changes, mortgage rate changes,
          new guides, and site improvements. All updates are sourced and dated for transparency.
        </p>
      </div>
        ...
        {updates.map((update, i) => (
          <article key={i} className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
            <div className="mb-2 flex items-center gap-3">
              ...
              <span
                className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${categoryColors[update.category] || "bg-zinc-100 text-zinc-800"}`}
              >
              ...
              <div className="flex flex-wrap gap-2">
                {update.relatedPages.map((page) => (
                  <Link
                    key={page.href}
                    href={page.href}
                    className="rounded-md bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-100"
                  >
                    {page.label}
                  </Link>
                ))}
